...
namespace RAC
{
	...
	std::vector<Real> ProcessPeakHighShelf(const int fs, const Real fc, const Real Q, const Real gain, const std::vector<Real>& input)
	{
	...
	TEST_CLASS(PeakHighShelf_Class)
	{
	public:
		...
		TEST_METHOD(Default)
		{
			...
			std::vector<Real> output = ProcessPeakHighShelf(fs, fc, Q, gain, input);
			;
		...
		TEST_METHOD(ProcessAudio)
		{
			...
			PeakHighShelf filter(fc, gain, Q, fs);
			...
			;
			for (int i = 0; i < output.size(); i++)
				Assert::AreEqual(output[i], filter.GetOutput(input[i], lerpFactor), EPS_TEST_LOW, L"Wrong output");
		}
