...
namespace RAC
{
	...
	std::vector<Real> ProcessPeakingFilter(const int fs, const Real fc, const Real Q, const Real gain, const std::vector<Real>& input)
	{
		...
		const Real norm = REAL_CONST(1.0) / (REAL_CONST(1.0) + alpha / A);
		const Real a1 = REAL_CONST(-2.0) * cosOmega * norm;
		const Real a2 = (REAL_CONST(1.0) - alpha / A) * norm;

		const Real b0 = (REAL_CONST(1.0) + alpha * A) * norm;
		const Real b1 = REAL_CONST(-2.0) * cosOmega * norm;
		const Real b2 = (REAL_CONST(1.0) - alpha * A) * norm;
	...
	public:
		...
		TEST_METHOD(ClearBuffers)
		{
			const int fs = 48000;
			const Real lerpFactor = REAL_CONST(0.5);
			const Real fc = REAL_CONST(1700.0);
		...
		TEST_METHOD(Process)
		{
			...
			auto outputData = Parse2Dcsv<Real>(filePath + "peakingFilterOutput.csv");
			...
			for (int i = 0; i < numTests; i++)
			{
				...
				for (int j = 0; j < numFrames; j++)
				{
					...
					Assert::AreEqual(outputData[i][j], out[j], EPS_TEST_MEDIUM, werrorchar);
				}
			}
		}
	};
