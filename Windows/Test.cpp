...
#pragma optimize("", off)
	...
	public:
		...
		TEST_METHOD(LTC)
		{
			...
			IEMConfig iemConfig = IEMConfig(DirectSound::check, 3, 0, 0, true, 0.0);
			...
			std::vector<IEMConfig> iemConfigs = { IEMConfig(DirectSound::check, 3, 0, 0, true, 0.0),
							IEMConfig(DirectSound::check, 0, 0, 0, true, 0.0) };
