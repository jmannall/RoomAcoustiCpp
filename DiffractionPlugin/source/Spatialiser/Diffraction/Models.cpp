...
{
	return abs((CalcHpm(0.0f, f) + CalcHpm(mPath->wData.z, f)) / complexF(4.0f, 0.0f));
}

complexF UDFA::CalcHpm(float z, float f)
{
	return CalcH(z, mPath->sData.t + mPath->rData.t, f) + CalcH(z, mPath->rData.t - mPath->sData.t, f);
}

complexF UDFA::CalcH(float z, float t, float f)
{
...
complexF UDFA::CalcUDFA(float f, float fc, float g)
{
	float alpha = 0.5f;
	float b = 1.44f;
	float Q = 0.2f;
	...
	return pow(pow(imUnit * f / fc, 2 / b) + pow(imUnit * f / (Q * fc), 1 / (powf(b, r))) + complexF(1.0f, 0.0f), -alpha * b / 2);
}
...
complexF UDFAI::CalcH(float z, float t, float f)
{
	float fc = front;
	float t1 = mPath->GetD(z) / SPEED_OF_SOUND;
...
void UTD::CalcUTD()
{
	...
	for (int i = 0; i < 4; i++)
	{
		complexF A = -exp(-imUnit * k[i] * dSR) * E[i] / temp;
...
complexF UTD::EqHalf(float t, const int i)
{
